...
<head>
    ...
    <title>Bin Status - Smart Waste Management System</title>
...
<body class="bg-gradient-to-br from-emerald-100 via-indigo-50 to-amber-100 dark:from-emerald-950 dark:via-slate-900 dark:to-indigo-950 text-gray-900 dark:text-white h-screen flex items-center justify-center p-4 lg:p-8 overflow-hidden relative transition-colors duration-500">
    ...
    <div class="w-full max-w-[1600px] h-full bg-white/30 dark:bg-slate-900/60 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(16,185,129,0.2)] dark:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] flex overflow-hidden border border-white/50 dark:border-slate-700/50 relative z-10">
        ...
        <aside id="sidebar" class="bg-transparent flex flex-col h-full transition-all duration-300 relative z-20 w-72 shrink-0 pt-6 pb-6 border-r border-white/30 dark:border-slate-700/50">
            ...
            <nav class="flex-1 overflow-y-auto py-2 px-4 space-y-2 custom-scrollbar">
                ...
                <a href="bin-status.php" class="nav-item group flex items-center px-4 py-3.5 rounded-2xl bg-white/50 dark:bg-slate-800/80 shadow-sm text-amber-600 dark:text-amber-500 font-bold relative overflow-hidden transition-all duration-200 backdrop-blur-md border border-white/60 dark:border-slate-600/50">
                    <div class="indicator absolute left-0 top-1/4 bottom-1/4 w-1 bg-amber-500 rounded-r-full transition-all duration-300"></div>
                <i class="fas fa-trash-alt w-6 text-center text-lg"></i>
                    <span class="ml-3 nav-label whitespace-nowrap transition-all duration-300 text-gray-900 dark:text-white">Bin Status</span>
            </a>
    ...
    <!-- Main Content -->
        <main id="mainContent" class="main-content flex-1 h-full overflow-y-auto custom-scrollbar transition-all duration-300 bg-white/30 rounded-tl-[2.5rem] rounded-bl-[2.5rem] shadow-[inset_10px_0_30px_rgba(255,255,255,0.4)] border-l border-white/50">
            <div id="bin-status" class="p-10">
                <div class="mb-10">
                    <h1 class="text-3xl font-extrabold text-gray-900 mb-1 drop-shadow-sm">Bin Status</h1>
                    <p class="text-gray-700 font-medium">Monitor all waste bins in real-time</p>
            </div>
            ...
            <!-- Filters -->
                ...
                <div class="flex flex-col md:flex-row gap-4">
                    <div class="flex-1 relative">
                            ...
                            <input type="text" id="binSearch" placeholder="Search by Bin ID or Location..." class="w-full pl-12 pr-4 py-2.5 bg-white/40 backdrop-blur-xl border border-white/60 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-amber-200 shadow-sm text-gray-900 placeholder-gray-500">
                    </div>
                        <select id="statusFilter" class="px-4 py-2.5 bg-white/40 backdrop-blur-xl border border-white/60 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-amber-200 shadow-sm text-gray-900">
                        <option value="all">All Status</option>
                        <option value="Normal">Normal</option>
                        <option value="Warning">Warning</option>
                        <option value="Full">Full</option>
                    </select>
                        <button class="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-400 text-white font-bold shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all">
            ...
            <!-- Table -->
                <div class="bg-white/30 backdrop-blur-2xl rounded-[1.5rem] shadow-[0_8px_32px_rgba(0,0,0,0.03)] border border-white/50 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead>
                                <tr class="bg-white/40 border-b border-white/50">
                                    <th class="px-6 py-4 text-left text-sm font-bold text-gray-900">Bin ID</th>
                                    <th class="px-6 py-4 text-left text-sm font-bold text-gray-900">Location</th>
                                    <th class="px-6 py-4 text-left text-sm font-bold text-gray-900">Fill Level</th>
                                    <th class="px-6 py-4 text-left text-sm font-bold text-gray-900">Status</th>
                                    <th class="px-6 py-4 text-left text-sm font-bold text-gray-900">Last Updated</th>
                            </tr>
                        </thead>
                            <tbody id="binTableBody" class="divide-y divide-white/40 text-gray-800 font-medium"></tbody>
                    </table>
                </div>
            </div>
                <div class="mt-4 text-sm font-medium text-gray-600 ml-2" id="binCount"></div>
        </div>
    </main>
    </div>
