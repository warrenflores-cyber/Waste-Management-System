...
<head>
    ...
    <title>Settings - Smart Waste Management System</title>
...
<body class="bg-gradient-to-br from-emerald-100 via-indigo-50 to-amber-100 dark:from-emerald-950 dark:via-slate-900 dark:to-indigo-950 text-gray-900 dark:text-white h-screen flex items-center justify-center p-4 lg:p-8 overflow-hidden relative transition-colors duration-500">
    ...
    <div class="w-full max-w-[1600px] h-full bg-white/30 dark:bg-slate-900/60 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(16,185,129,0.2)] dark:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] flex overflow-hidden border border-white/50 dark:border-slate-700/50 relative z-10">
        ...
        <aside id="sidebar" class="bg-transparent flex flex-col h-full transition-all duration-300 relative z-20 w-72 shrink-0 pt-6 pb-6 border-r border-white/30 dark:border-slate-700/50">
            ...
            <nav class="flex-1 overflow-y-auto py-2 px-4 space-y-2 custom-scrollbar">
                ...
                <a href="settings.php" class="nav-item group flex items-center px-4 py-3.5 rounded-2xl bg-white/50 dark:bg-slate-800/80 shadow-sm text-amber-600 dark:text-amber-500 font-bold relative overflow-hidden transition-all duration-200 backdrop-blur-md border border-white/60 dark:border-slate-600/50">
                ...
                <i class="fas fa-cog w-6 text-center text-lg"></i>
                    <span class="ml-3 nav-label whitespace-nowrap transition-all duration-300 text-gray-900 dark:text-white">Settings</span>
            </a>
    ...
    <!-- Main Content -->
        <main id="mainContent" class="main-content flex-1 h-full overflow-y-auto custom-scrollbar transition-all duration-300 bg-white/30 rounded-tl-[2.5rem] rounded-bl-[2.5rem] shadow-[inset_10px_0_30px_rgba(255,255,255,0.4)] border-l border-white/50">
            <div id="settings-page" class="p-10">
                <div class="mb-10">
                    <h1 class="text-3xl font-extrabold text-gray-900 mb-1 drop-shadow-sm">Settings</h1>
                    <p class="text-gray-700 font-medium">Manage your system preferences</p>
            </div>

                <div class="bg-white/30 backdrop-blur-2xl rounded-[1.5rem] p-6 shadow-[0_8px_32px_rgba(0,0,0,0.03)] border border-white/50">
                <div class="space-y-6">
                    <!-- Notification Settings -->
                    <div>
                            <h2 class="text-xl font-extrabold text-gray-900 mb-4 drop-shadow-sm">Notification Settings</h2>
                            <div class="flex items-center justify-between py-3 border-b border-white/40 last:border-b-0">
                                <label for="notificationEmail" class="text-gray-800 font-medium">Notification Email Address</label>
                                <input type="email" id="notificationEmail" placeholder="admin@ecosync.com" class="w-64 px-4 py-2.5 bg-white/40 backdrop-blur-xl border border-white/60 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-amber-200 shadow-sm text-gray-900 placeholder-gray-500 text-right">
                            </div>
                            <div class="flex items-center justify-between py-3 border-b border-white/40 last:border-b-0">
                                <div>
                                    <label for="notificationFrequencyValue" class="text-gray-800 font-medium block">Notification Frequency</label>
                                    <p class="text-xs text-gray-500 mt-0.5">Set to 0 for "only once per fill"</p>
                                </div>
                                <div class="flex items-center gap-2">
                                    <input type="number" id="notificationFrequencyValue" min="0" placeholder="0" class="w-20 px-4 py-2.5 bg-white/40 backdrop-blur-xl border border-white/60 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-amber-200 shadow-sm text-gray-900 placeholder-gray-500 text-center">
                                    <select id="notificationFrequencyUnit" class="w-32 px-4 py-2.5 bg-white/40 backdrop-blur-xl border border-white/60 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-amber-200 shadow-sm text-gray-900">
                    ...
                    <div class="flex justify-end pt-4">
                            <button onclick="saveSettings()" class="px-6 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-400 text-white font-bold shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all">
                            Save Settings
                        </button>
                    </div>
                </div>
